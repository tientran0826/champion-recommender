...
# Convenience functions for Champion Recommender Project
class ChampionS3Storage(S3Operator):
    """Specialized S3 operator for Champion Recommender data"""

    def save_player_data(self, puuid: str, region: str, player_data: Dict) -> bool:
        """Save player data with organized key structure"""
        date_str = datetime.now().strftime('%Y-%m-%d')
        key = f"players/{region}/{date_str}/{puuid}.json"
        ...
        return self.upload_json(key, player_data, metadata)

    def get_player_data(self, puuid: str, region: str, date: str = None) -> Optional[Dict]:
        """Get player data by PUUID and region"""
        if not date:
            date = datetime.now().strftime('%Y-%m-%d')

        key = f"players/{region}/{date}/{puuid}.json"
        return self.download_json(key)

    def save_match_data(self, match_id: str, region: str, match_data: Dict) -> bool:
        """Save match data"""
        date_str = datetime.now().strftime('%Y-%m-%d')
        key = f"matches/{region}/{date_str}/{match_id}.json"

        metadata = {
            'region': region,
            'match_id': match_id,
            'game_duration': str(match_data.get('info', {}).get('gameDuration', 0))
        }

        return self.upload_json(key, match_data, metadata)

    def save_processing_summary(self, summary_data: Dict) -> bool:
        """Save daily processing summary"""
        timestamp = datetime.now()
        date_str = timestamp.strftime('%Y-%m-%d')
        time_str = timestamp.strftime('%H-%M-%S')

        key = f"summaries/{date_str}/summary_{time_str}.json"
        return self.upload_json(key, summary_data)

    def list_players_by_region(self, region: str, date: str = None) -> List[str]:
        """List all player PUUIDs for a region and date"""
        if not date:
            date = datetime.now().strftime('%Y-%m-%d')

        prefix = f"players/{region}/{date}/"
        object_keys = self.list_objects(prefix=prefix)

        # Extract PUUIDs from filenames
        puuids = []
        for key in object_keys:
            if key.endswith('.json'):
                puuid = key.split('/')[-1].replace('.json', '')
                puuids.append(puuid)
